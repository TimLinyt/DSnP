...
{
   // TODO...
   for (CmdMap::const_iterator cit = _cmdMap.cbegin(); 
        cit != _cmdMap.cend(); cit ++) cit->second->help();
}
...
{   
   ...
   if (e == 0) cerr << "Illegal command!! \"( " << cmd << " )\"";
   else if (n != string::npos) 
      option = str.substr(str.find_first_not_of(' ', n));
...
CmdParser::listCmd(const string& str)
{
   // TODO...
   string first_w;
   size_t n = myStrGetTok(str, first_w);
   if (first_w == "") { //1 null or ' '
      cout << _readBufPtr;
      int counter = 0;
      ...
      for (CmdMap::iterator i = _cmdMap.begin(); i != _cmdMap.end(); i++) {
         string cmd = i->first + i->second->getOptCmd();
         if (cmd.size() < first_w.size()) continue;
         if (myStrNCmp(cmd, first_w, first_w.size()) == 0) { 
            matches.push_back(i->first + i->second->getOptCmd());
         }
      }
      if (matches.size() == 0) { //4 no matches
         mybeep();
         return;
      }
      else if (matches.size() == 1) {  //3 one match
         for (int i = matches[0].size()-first_w.size(); i > 0; i--)
            insertChar(matches[0][matches[0].size()-i]);
         insertChar(' ');
         return;
      }
      else { //2 multi matches
         cout << _readBufPtr;
         for (int i = 0; i < matches.size(); i++) {
            if (i%5 == 0) cout << endl;
            cout << setw(12) << left << matches[i];
         }
      }
   }
   else { //not first
      CmdExec* e = getCmd(first_w); //5 already match
      if (e == 0) { //6 no match
         mybeep();
         return;
      }
      cout << _readBufPtr << endl;
      e->usage(cout);
   }
   cout << endl;
   printPrompt();
...
CmdParser::getCmd(string cmd)
{
   ...
   for (CmdMap::iterator it = _cmdMap.begin(); it != _cmdMap.end(); it++) {
      if (myStrNCmp(it->first + it->second->getOptCmd(), 
                    cmd, it->first.size()) == 0) e = it->second;
   }
   return e;
}
