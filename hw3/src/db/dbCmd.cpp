...
DBAppendCmd::exec(const string& option)
{
   ...
   vector<int> append;
   for (int i = 1; i <= (doRow? dbtbl.nCols(): dbtbl.nRows()); i++) {
      int element;
      if (i >= options.size() || options[i] == "-") element = INT_MAX;
      else if (!myStr2Int(options[i], element))
         return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[i]);     
      append.push_back(element);
   }

   if (doRow) dbtbl.addRow(DBRow(append));
   else dbtbl.addCol(append);
