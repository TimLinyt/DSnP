...
CmdParser::readCmdInt(istream& istr)
{
   ...
   while (1) {
      ...
      switch (pch) {
         ...
         case BACK_SPACE_KEY : if(_readBufPtr == _readBuf) {mybeep(); break;}
...
CmdParser::moveBufPtr(char* const ptr)
{
   if (ptr < _readBuf || ptr > _readBufEnd) {
      ...
      for (int i = int(_readBufPtr - ptr); i > 0; i--)
         cout << '\b' ;
   else if (ptr > _readBufPtr)
      for (int i = 0; i < int(ptr - _readBufPtr); i++)
         cout << _readBufPtr[i]; 
   _readBufPtr = ptr;
   return true;
}
...
{
   assert(repeat >= 1);
   for (int i = int(_readBufEnd-_readBufPtr); i >= 0; i--)
...
{
   moveBufPtr(_readBuf);
   for (int i = _readBufEnd - _readBuf; i > 0; i--)
      deleteChar();
}           
...
{
   int start = 0, end = 0; 
   while (*(_readBuf + start) == ' ') start ++;
   while (*(_readBufEnd - end - 1) == ' ') end ++;
   if(_tempCmdStored) {
      _history.pop_back();
      _tempCmdStored = false;   
   }
   if (start != (_readBufEnd - _readBuf)) {
      string add(_readBuf + start, (_readBufEnd - end) - (_readBuf + start));
