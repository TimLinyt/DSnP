...
int main(int argc, char** argv)
{
   ...
   for(size_t i = 0, s = validGate.size() - 1; i < OO; ++i){
      size_t idx = ((s-i) < 0 ? (rand() % s) : (s-i));
      outf << LITERAL(validGate[idx]) << endl;
   }
   ...
   for(size_t i = 0; i < AA; ++i){
      ...
      size_t i1 = validInGate[(rand() % (II + i))];
      size_t i2 = validInGate[(rand() % (II + i))];
   ...
   // comment
   outf << "c" << endl;
   outf << "AAG(DAG) by Chen Hao Hsu" << endl;
